...
//                   onClick={() => setSelectedDriver(row.original)}
//                 >
...
const DriversTable = () => {
  ...
  const handleUpdate = driver => {
    setUpdatingDriver(driver); // Set the driver to open the update form
  };
  ...
  return (
    <div className='overflow-x-auto'>
      <div className='bg-white shadow-lg rounded-lg overflow-hidden'>
        {/* Wrap the table to enable horizontal scrolling */}
        <div className='min-w-full'>
          <table className='min-w-full bg-white'>
            <thead>
              {table.getHeaderGroups().map(headerGroup => (
            ...
            <tbody>
              {table.getRowModel().rows.map(row => (
                <tr
                  ...
                  className='hover:bg-blue-50 transition-colors duration-200 cursor-pointer'
                  onClick={() => {
                    // Do nothing here if the row should not trigger the dialog
                  }}
                >
                  {row.getVisibleCells().map(cell => (
                    <td
